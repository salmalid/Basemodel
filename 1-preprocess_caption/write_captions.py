"""
 format:
  Single pathology  : "Frontal CXR showing Pneumothorax"
  Multiple          : "Frontal CXR showing Cardiomegaly and Pleural Effusion"
  Three+            : "Frontal CXR showing Atelectasis, Consolidation, and Support Devices"
  ...
  - Support Devices is treated as a regular pathology (included in the list)
"""
import json
import os
...
REPORT_FILE = PARENT_ROOT / "data_preprocessing" / "report_fixed.json"
...
def _join_pathologies(active: list) -> str:
    if len(active) == 1:
        return active[0]
    if len(active) == 2:
...
def build_caption(record: dict):
    path = record.get("path_to_image", "")
    ...
    return f"Frontal CXR showing {_join_pathologies(active)}"
...
for r in records:
    ...
    image_id   = parts[3].replace(".jpg", "") if len(parts) > 3 else ""
